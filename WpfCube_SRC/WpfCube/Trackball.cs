...
namespace WpfCube
{
    public delegate void QuaternionChangeEventHandler(object sender, Quaternion qt);
    public class Trackball
    {
        public event QuaternionChangeEventHandler OnQuaternionChanged;
        public ScreenSpaceLines3D _axisX;
        public ScreenSpaceLines3D _axisY;
        public ScreenSpaceLines3D _axisZ;
        public Trackball()
        {
        ...
        private void UpdateSlaves(Quaternion q, double s, Vector3D t)
        {
            ...
            if (_slaves != null)
            {
                ...
                foreach (Viewport3D i in _slaves)
                {
                    ...
                    //i.Camera.Transform = tg;//CameraÇâÒì]Ç∑ÇÈèÍçá
                    //CubeÇâÒì]Ç∑ÇÈèÍçá
                    foreach (ModelVisual3D mv in i.Children)
                    {
                        if (mv.Content != null)
                        {
                            continue;//LightÇÕâΩÇ‡ÇµÇ»Ç¢
                        }
                        mv.Transform = tg;
                    }
                    _axisX.Transform = tg;
        ...
        private void MouseMoveHandler(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            if (el.IsMouseCaptured)
            {
                ...
                //if (OnQuaternionChanged != null)
                //{
                //    OnQuaternionChanged(this, _q);
                //}
        ...
        //public Quaternion Quaternion { get { return _q; } }
    }
}
